...
import { Card, CardBody, CardHeader, Tabs, Tab, Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, Chip } from '@nextui-org/react';
  ...
  const getChartData = () => {
    ...
    if (!Array.isArray(transactions) || transactions.length === 0) {
      return {
        labels: [],
        datasets: [{
          label: chartType === 'value' ? 'Transaction Value (USD)' : 'Transaction Count',
          data: [],
    ...
    <Card className="w-full">
      <CardHeader className="flex justify-between">
        ...
        <div className="flex gap-4">
          <Tabs selectedKey={timeRange} onSelectionChange={(key) => setTimeRange(key as any)}>
            <Tab key="24h">24H</Tab>
            <Tab key="7d">7D</Tab>
            <Tab key="30d">30D</Tab>
            <Tab key="all">All</Tab>
          </Tabs>
          <Tabs selectedKey={chartType} onSelectionChange={(key) => setChartType(key as any)}>
            ...
            {transactions.map(tx => (
              <TableRow key={tx.signature}>
                ...
                <TableCell>
                  ...
                  {tx.usdValue && <span className="text-sm text-default-400"> (${tx.usdValue.toFixed(2)})</span>}
                </TableCell>
